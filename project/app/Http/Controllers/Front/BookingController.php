...
public function cart_submit(Request $request)
{
    ...
    $total_guests = $request->adult + $request->children;
    if ($total_guests > $room->total_guests) {
        return redirect()->back()->with('error', 'Total number of guests exceeds the maximum allowed for this room')->withInput();
    }
    ...
    public function checkout()
    {
        if(!Auth::guard('customer')->check()) {
            return redirect()->back()->with('error', 'You must have login as customer in order to checkout!');
        }
    ...
    {
        if(!Auth::guard('customer')->check()) {
            return redirect()->back()->with('error', 'You must have to login as customer in order to checkout!');
        }
    ...
    public function paypal($final_price)
    {
        ...
        if($result->state == 'approved')
        {
            ...
            for($i=0;$i<count($arr_cart_room_id);$i++)
            {
                ...
                $customer_data = Customer::where('email', session('billing_email'))->first();

                // Update the customer data
                $customer_data->name = session('billing_name');
                $customer_data->email = session('billing_email');
                $customer_data->phone = session('billing_phone');
                $customer_data->country = session('billing_country');
                $customer_data->address = session('billing_address');
                $customer_data->state = session('billing_state');
                $customer_data->city = session('billing_city');
                $customer_data->zip = session('billing_zip');
            ...
            return redirect()->route('customer_order_view')->with('success', 'Payment is successful');
        }
        else
        {
    ...
    public function stripe(Request $request, $final_price)
    {
        ...
        for($i=0;$i<count($arr_cart_room_id);$i++) {
            ...
            $message .= '<br>Room Name: '.$r_info->name;
            $message .= '<br>Price Per Day: $'.$r_info->price;
            $message .= '<br>Checkin Date: '.$arr_cart_checkin_date[$i];
            $message .= '<br>Checkout Date: '.$arr_cart_checkout_date[$i];
            $message .= '<br>Adult: '.$arr_cart_adult[$i];
            $message .= '<br>Children: '.$arr_cart_children[$i].'<br>';
